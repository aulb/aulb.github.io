...
} from './playlistTopGenres.module.css'

/* 
Find top 10 genres per playlist
*/

const PlaylistTopGenres = ({limit}) => {
  const playlistsGenres = PlaylistsGenres
  ...
  playlistsGenres.forEach(playlistGenre => {
    playlistGenre.genres.sort((genre1, genre2) => genre2.count - genre1.count || genre1.pca - genre2.pca)
  })
  return (
    <div>
    {
        ...
        return (
            <div key={`${playlist.name}-spotgenre`} className={playlistCubeContainer}>
            {
                ...
                return (
                    <div 
                    key={`${playlist.name}-${genre}-${index}`} title={genre["name"]} 
                    className={playlistCube} 
                    style={{
                        background: rgb,
...
export default PlaylistTopGenres
